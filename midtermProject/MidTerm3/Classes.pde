...
class Level {
  int numOfBlocks, rows, yPos, xGap, yGap, gameLevel, blocksRemaining;
  Block[] blocks;
  boolean win;
  ...
  void endOfLevel(){
    if (lives[gameLevel] != 0 && blocksRemaining == 0 ){
      currLevel ++;
      startLevel = false;
      gameBall.reset();
      pause = true;
...
class Ball {
  ...
  void reset() { // Reset the ball's location back to the original point
    lives[currLevel-1] --; //Decrement the number of lives by 1
    inMotion = false;
    startLevel = false;
    x = BALLX;
    y = BALLY;
        ...
        if(x+radius >= level.blocks[i].x && x-radius <= (level.blocks[i].x + BLOCKWIDTH) && y+radius >= level.blocks[i].y && y-radius <= (level.blocks[i].y + BLOCKHEIGHT)){
          if (level.blocks[i].obstacle == false){
            level.blocks[i].damage ++;
             if (level.blocks[i].damage == 1) {
               level.blocks[i].show = false;
               level.blocksRemaining --;
