...
class Level {
  ...
  void score() { // Display the game score
    ...
    textAlign(LEFT);
    text("Score: " + levels[currLevel].score, 0, textY);
  }
  ...
  void remainingLives() {
    ...
    textAlign(RIGHT);
    text("Lives: " + levels[currLevel].lives, width-100, textY);
  }
  void endOfLevel(){
...
class Ball {
  ...
  void reset() { // Reset the ball's location back to the original point
    levels[currLevel].lives --; //Decrement the number of lives by 1
    inMotion = false;
    startLevel = false;
    x = BALLX;
    y = BALLY;
  ...
  void collision() {
    ...
    for (int i=0; i< levels[currLevel].numOfBlocks; i++) {
      // If the ball collides with a block
      if (levels[currLevel].blocks[i].show == true) {
        if(x+radius >= levels[currLevel].blocks[i].x && x-radius <= (levels[currLevel].blocks[i].x + BLOCKWIDTH) && y+radius >= levels[currLevel].blocks[i].y && y-radius <= (levels[currLevel].blocks[i].y + BLOCKHEIGHT)){
          if (levels[currLevel].blocks[i].obstacle == false){
            levels[currLevel].blocks[i].damage ++;
             if (levels[currLevel].blocks[i].damage == 1) {
               levels[currLevel].blocks[i].show = false;
               levels[currLevel].blocksRemaining --;
          ...
          if(tempX+radius >= levels[currLevel].blocks[i].x && tempX-radius <= (levels[currLevel].blocks[i].x + BLOCKWIDTH) && y+radius >= levels[currLevel].blocks[i].y && y-radius <= (levels[currLevel].blocks[i].y + BLOCKHEIGHT)){
            ySpeed = -ySpeed;
          }
          else {
            // If a change in the Y Speed of the ball still presents a collision with the same block, change the X Speed
            if(x+radius >= levels[currLevel].blocks[i].x && x-radius <= (levels[currLevel].blocks[i].x + BLOCKWIDTH) && tempY+radius >= levels[currLevel].blocks[i].y && tempY-radius <= (levels[currLevel].blocks[i].y + BLOCKHEIGHT)){
              xSpeed = -xSpeed;
            }
          } 
        }
